...
class TextUI:
    ...
    def run(self):
        ...
        self.view.display(f"/// \U0001F916 {bot_ver} loaded. Waiting for command. \"help\" to show list of all commands.")
        ...
        while True:
            ...
            if cmd == unknown_handler:
                self.view.display(f"/// Invalid command. Did you mean '{find_closest_command(user_input)}'?")
                continue
...
def display_result(result):
    if result.startswith("///"):
        ...
        row_width = max(header_width, max(len(line) for line in content), len(footer))

        print("+" + "-" * (row_width + 2) + "+")
        print("| " + header.center(row_width) + " |")
        print("+" + "-" * (row_width + 2) + "+")

        for line in content:
            print("| " + line.ljust(row_width) + " |")

        print("+" + "-" * (row_width + 2) + "+")
        print("| " + footer.center(row_width) + " |")
        print("+" + "-" * (row_width + 2) + "+")


if __name__ == "__main__":
    console_view = ConsoleView()
    text_ui = TextUI(console_view)
    text_ui.run()
